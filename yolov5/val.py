...
def process_batch(detections, labels, iouv):
    ...
    for i in range(len(iouv)):
        ...
        if x[0].shape[0]:
            ...
            for match in matches:  # Store IoU per class
                class_id = int(labels[int(match[0]), 0])  # Class ID from labels
                class_iou[class_id].append(match[2])  # Append IoU value
            correct[matches[:, 1].astype(int), i] = True
    mean_class_iou = {cls: float(np.mean(iou_values)) for cls, iou_values in class_iou.items()}
    for cls, iou in mean_class_iou.items():
        print(f"Class {cls}: Mean IoU = {iou:.3f}")
